...
(define (reg-make-type r-name r-type #:store [store #t])
	...
	(cond
		...
		[(equal? primary-type "boolean") (reg-make-boolean r-name #:store store)]
		[(equal? primary-type "integer") (reg-make-integer r-name #:store store)]
		[(equal? primary-type "array") (reg-make-vector r-name secondary-type #:store store)]
...
(define (do-solve json-obj)
	...
	(for ([d-block json-normal-blocks])
		...
		(define scoped-insts 
			(for/fold ([tmp-list (list)]) ([tmp-elem d-insts])
	...
	(define angelic-model (solve (assert (register-ref "ANGELIC-FLAG"))))
